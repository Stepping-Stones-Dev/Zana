...
if (!admin.apps.length) {
  try {
    ...
    if (svc) {
      ...
      admin.initializeApp();
      // eslint-disable-next-line no-console
      console.warn("firebase-admin initialized without explicit credentials. Provide FIREBASE_SERVICE_ACCOUNT or GOOGLE_APPLICATION_CREDENTIALS for production.");
    }
  } catch (e) {
    // eslint-disable-next-line no-console
