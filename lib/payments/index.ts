...
export const PesapalProvider: PaymentProvider = {
  ...
  async initiateMandate({ email, planId, billing, locale, phone }) {
    ...
    const amounts = pricing.computePlanAmounts ? pricing.computePlanAmounts(planId, billing) : { monthlyKES: 0, yearlyKES: 0 };
  ...
  const pubBase = process.env.PUBLIC_BASE_URL || "http://localhost:3000";
    ...
    if (billing === "monthly") endDate.setFullYear(endDate.getFullYear() + 1);
  ...
  try { orderJson = JSON.parse(orderText); } catch {}
...
// Wrapper so "card" flows can use Pesapal hosted checkout too
// Note: We route both mobile money and card to Pesapal hosted checkout.
  ...
  async initiateMandate({ email, planId, billing, locale }) {
    try {
      ...
      if (clientId && clientSecret && ppPlanId) {
        ...
        if (approve) return { simulated: false, hostedUrl: approve };
      }
    } catch (e) {
      // ignore and fall back to simulated
    }
    return { simulated: true, hostedUrl: "#" };
  },
  async charge() {
