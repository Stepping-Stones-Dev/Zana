...
try { sh('git fetch origin main --depth=1'); } catch { /* ignore */ }
try { sh('git rev-parse --verify origin/main'); } catch { base = 'HEAD~1'; }
...
if (mode === 'lint'){
  ...
  if (pkgs.size === 0){
    ...
    const filters = Array.from(pkgs).map(p=>`--filter=@zana/${p}`).join(' ');
