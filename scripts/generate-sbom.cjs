...
const { writeFileSync, mkdtempSync, readFileSync, rmSync, existsSync, readdirSync, statSync, mkdirSync } = require('node:fs');
...
function loadCache(name, pkgJson){
  ...
  } catch { /* cache read ignore */ }
  return null;
}

function writeCache(name, pkgJson, report){
  try { writeFileSync(join(cacheRoot(), cacheKey(name, pkgJson)), JSON.stringify(report)); } catch { /* ignore cache write */ }
}
...
function run(cmd, cwd){
  ...
  for (const line of stderr.split(/\r?\n/)){
      ...
      if (/^npm (ERR|error)/i.test(line.trim())) { suppressed++; continue; }
      filtered.push(line);
    }
    if (filtered.length) console.error('[sbom] command issues (filtered) for', cmd, '\n'+filtered.join('\n'));
...
function generateReport(name, dir, cmdBases, tmp){
  ...
  const cmd = `${base} --output-format json --output-file ${outFile} ${extra}`.trim();
      ...
      if (existsSync(outFile)){
        try {
          const st = statSync(outFile); if (st.size === 0) throw new Error('empty');
          const json = loadJson(outFile);
          if(!res.ok) console.warn('[sbom] soft-fail treated as success for', name);
          writeCache(name, pkgJson, json);
          return json;
  } catch { /* parse failed; continue attempts */ }
      }
      // If the stderr indicates pnpm ls option incompatibility, break to next base (fallback to npx)
  if (res.stderr && /Unknown option: 'all'|failed to parse npm-ls response|npm-ls exited/i.test(res.stderr)) {
        if (!incompatible) console.warn('[sbom] detected pnpm ls incompatibility; switching command base');
        incompatible = true;
  ...
  for (const c of components){
    ...
    if (Array.isArray(c.licenses)){
      for (const l of c.licenses){
        if (l?.license){
          if (l.license.id) licenseIds.push(l.license.id); else if (l.license.name) licenseIds.push(l.license.name);
        } else if (l?.expression) licenseIds.push(l.expression);
      }
    } else if (c.license){
      if (c.license.id) licenseIds.push(c.license.id); else if (c.license.name) licenseIds.push(c.license.name);
    }
    if (!licenseIds.length) licenseIds = ['(unknown)'];
    for (const lid of licenseIds){ licenseCounts.set(lid, (licenseCounts.get(lid)||0)+1); }
  }
  const total = components.length || 1;
  const sortEntries = (map)=>Array.from(map.entries()).sort((a,b)=>b[1]-a[1]);
  return {
    totalComponents: total,
    types: sortEntries(typeCounts).map(([k,v])=>({ type:k, count:v, pct: +( (v/total)*100 ).toFixed(2) })),
    licenses: sortEntries(licenseCounts).map(([k,v])=>({ license:k, count:v, pct: +( (v/total)*100 ).toFixed(2) }))
  };
}

function emitSummary(components){
  const summary = deriveSummary(components);
  const top = (list,key)=>list.slice(0,10).map(e=>`${e[key]}:${e.count}(${e.pct}%)`).join(', ');
  ...
  if (SUMMARY_MODE === 'json'){
    try { writeFileSync('sbom-summary.json', JSON.stringify(summary, null, 2)); console.log('[sbom] wrote sbom-summary.json'); } catch(e){ console.warn('[sbom] failed to write sbom-summary.json', e.message); }
  }
}
...
function main(){
  ...
  for (const ws of workspaces){
    try {
      const report = generateReport(require(join(ws,'package.json')).name, ws, [cmdBasePrimary, cmdBaseFallback], tmp);
      reports.push(report);
  ...
  try {
    ...
    const rReport = generateReport(rootName==='zana'?'root':rootName, root, [cmdBasePrimary, cmdBaseFallback], tmp);
    reports.push(rReport);
    console.log('[sbom] root scan succeeded');
  } catch(e){ console.warn('[sbom] root scan failed', e.message); }
  if (!reports.length){
