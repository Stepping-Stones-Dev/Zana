...
function runAudit() {
  ...
  try {
    ...
    if (res.status !== 0 && !res.stdout) {
      console.warn('[security] pnpm audit failed (non-critical)');
      return [];
    }
    ...
    if (Array.isArray(data)) {
      for (const v of data) {
        if (v?.advisory) vulns.push(v.advisory);
      }
...
function runOSV() {
  ...
  try {
    ...
    if (res.status !== 0 && !res.stdout) {
      console.warn('[security] OSV scanner failed (non-critical)');
      return [];
    }
    const data = JSON.parse(res.stdout || '{}');
...
function writeCombinedSummary(mode, allV, failing = []) {
  ...
  const summary = {
    ...
    bySource: allV.reduce((acc, v) => {
      acc[v.source] = (acc[v.source] || 0) + 1;
      return acc;
    }, {}),
    severities: allV.reduce((acc, v) => {
  ...
  try {
    ...
    console.log('[security] wrote security-summary.json');
  } catch (e) {
    console.warn('[security] failed to write security-summary.json', e.message);
  }
}
...
function main() {
  ...
  console.log(
    '[security] starting composite scan (failOn=' +
      (failOn || 'none') +
      ', refresh=' +
      refresh +
      ', summary=' +
