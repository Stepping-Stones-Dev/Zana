...
export type AppEntry = { name: string; url: string; module: string };
...
export function registerApps(entries: AppEntry[]) {
  entries.forEach(registerApp);
}
...
export const RemoteLoader: React.FC<RemoteLoaderProps> = ({ app, className }) => {
  ...
  const RemoteComp = React.useMemo(
    () =>
      isClient && entry
        ? React.lazy(() =>
            // @ts-ignore
            (window as any)[entry.name].get(entry.module).then((factory: any) => {
              const Mod = factory();
  ...
  if (error)
    return (
      <Card className="p-6 text-center text-red-600 font-semibold">{error}</Card>
    );
    ...
    return (
      <Card className="flex flex-col items-center justify-center p-8">
        ...
        <div className="mt-4 text-lg font-medium text-gray-700">
          Loading <span className="font-bold">{app}</span>…
        </div>
      </Card>
    );
