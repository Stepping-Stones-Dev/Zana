...
// Router not needed; we persist locale and let next navigation pick it up
...
export function FloatingLanguage() {
  ...
  const change = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setLocale(nextLocale); // instant client-side switch via provider
    setNextLocaleCookie(nextLocale);

  // Do not navigate now; NEXT_LOCALE cookie ensures next navigation uses it
  };
